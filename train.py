...
if not cfg.RESUME_EPOCH:
    ...
    if not cfg.model_name.startswith('efficientnet'):
        ...
        model = cfg.MODEL_NAMES[cfg.model_name](cfg.model_name,
...
if cfg.RESUME_EPOCH:
    print(' ******* Resume training from {}  epoch {} *********'.format(
        cfg.model_name, cfg.RESUME_EPOCH))
    model = load_checkpoint(
        os.path.join(save_folder, 'epoch_{}.pth'.format(cfg.RESUME_EPOCH)))
...
for iteration in range(start_iter, max_iter):
    ...
    if iteration % 10 == 0:
        print('Epoch:' + repr(epoch) + ' || epochiter: ' +
              ...
              (loss.item()) + 'ACC: %.3f ||' % (train_acc * 100) + 'LR: %.8f' %
              (lr))
